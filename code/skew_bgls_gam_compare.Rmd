...
```{r read_data}
df <- read_tsv(file = 'C:/Home/SW_Specialist/Skew0405/resid_skew_anal/data/skew_data.txt') %>% 
...
df$HUC        <- rep('05', nrow(df))
ndx04         <- which(substr(df$Station,1,2) == '04')
df$HUC[ndx04] <- '04'
...
df %>% 
   mutate(HUC_name = ifelse(HUC == '04', 'Great Lakes', 'Ohio River')) %>% 
   ggplot( aes(x = HUC_name, y = Residual_skew)) +
   geom_boxplot(notch = TRUE, varwidth = TRUE) +
   geom_hline( yintercept = 0, color = 'red', linetype = 'dashed') +
   theme_few() +
   labs(x = 'Hydrologic Region', y = 'Skew Residual from Constant B-GLS')
